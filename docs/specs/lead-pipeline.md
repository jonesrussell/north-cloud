...
Recall verification for signal-crawler maintenance work: run `task recall:check -- --timeout 5m` from `signal-crawler/` to fetch the current HN and jobs adapter inputs once, then compare accepted counts at one vs two keyword matches without POSTing to Waaseyaa or touching dedup state. Use `--json` when posting issue evidence; the diagnostic also reports top one-hit phrases so review can distinguish sparse keyword coverage from a threshold-policy problem. A 2026-04-25 live run for #676 scanned HN plus jobs boards and found `hn-hiring` 3 -> 0 and `remoteok` 4 -> 0 at the two-keyword gate; all other adapters had zero candidates at either threshold. Recommendation from that sample: keep the shared two-keyword classifier contract, but treat signal-crawler job recall as a keyword-list/source-quality follow-up before relaxing the threshold per adapter.
...
Dry-run validation (post-deploy): `tools/validate-org-attribution` queries ES `_count` for populated `organization_name_normalized` across `*_classified_content` (need-signal documents) and `rfp_classified_content`, optionally filtering to recent documents with `--since` (RFC3339 timestamp or duration such as `24h`, applied to top-level `crawled_at`). It exits non-zero if the combined populated rate is below the configured threshold (default 0.25). The `.github/workflows/validate-org-attribution.yml` GitHub Action runs the validator daily (14:05 UTC) against production ES via the deploy host with `--since 24h`, reporting per-producer and aggregate rates to the job summary. A failure (rate below threshold) fails the workflow loudly; `workflow_dispatch` allows on-demand runs with a custom threshold and since window.

The threshold remains intentionally low until 14 consecutive days of clean validator runs on `--since 24h` have been recorded. Once the recent-doc rate is stable, raise the threshold to 0.80+. #639 remains closed as the wiring is complete.
