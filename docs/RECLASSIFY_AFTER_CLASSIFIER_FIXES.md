...
Run on your server (e.g. in `screen` or `tmux`). Replace `AUTH_PASSWORD` with the real auth password. Uses curl container + port **8070** (classifier listens on 8070; classifier container cannot reach its own localhost).
