...
<template>
  <div class="space-y-6">
    <div class="flex items-center gap-4">
      <Button variant="ghost" size="icon" @click="router.push('/scheduling/sources')">
        <ArrowLeft class="h-5 w-5" />
      </Button>
      <div>
        <h1 class="text-3xl font-bold tracking-tight">{{ isEdit ? 'Edit Source' : 'New Source' }}</h1>
        <p class="text-muted-foreground">{{ isEdit ? 'Update source configuration' : 'Add a new content source' }}</p>
      </div>
    </div>
      ...
      <CardContent>
        <form @submit.prevent="saveSource" class="space-y-4">
          <div>
            ...
            <Input v-model="form.name" placeholder="My News Source" />
          </div>
          ...
          <div class="flex items-center gap-2">
            <input type="checkbox" id="enabled" v-model="form.enabled" class="h-4 w-4" />
          ...
          <div class="flex justify-end gap-3 pt-4">
            <Button type="button" variant="outline" @click="router.push('/scheduling/sources')">
              Cancel
            </Button>
