    ...
    <div v-else>
      ...
      <div class="bg-white shadow rounded-lg p-6 mb-8">
        <div class="flex items-center justify-between">
          <div>
            <h2 class="text-lg font-medium text-gray-900 mb-1">Last Sync</h2>
      ...
      <!-- Cities Breakdown -->
      <div class="bg-white shadow rounded-lg p-6 mb-8">
        <h2 class="text-lg font-medium text-gray-900 mb-4">Statistics by City</h2>
        <div v-if="stats.cities && stats.cities.length > 0" class="space-y-4">
          <div
          ...
          >
            <div class="flex items-center justify-between mb-2">
              <h3 class="text-sm font-semibold text-gray-900">{{ city.name }}</h3>
            </div>
            <div class="grid grid-cols-3 gap-4 mt-2">
              <div>
                <span class="text-xs text-gray-500">Posted</span>
                <p class="text-lg font-semibold text-green-600">{{ city.posted }}</p>
              </div>
              <div>
                <span class="text-xs text-gray-500">Skipped</span>
                <p class="text-lg font-semibold text-yellow-600">{{ city.skipped }}</p>
              </div>
              <div>
                <span class="text-xs text-gray-500">Errors</span>
                <p class="text-lg font-semibold text-red-600">{{ city.errors }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="bg-white shadow rounded-lg p-6">
        <h2 class="text-lg font-medium text-gray-900 mb-4">Success Rate</h2>
        <div class="space-y-4">
          <div class="flex items-center justify-between">
