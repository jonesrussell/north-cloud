...
const staticPipeline = `Crawl: Crawler fetches pages and writes to Elasticsearch indexes named {source}_raw_content with classification_status=pending. Classify: Classifier reads raw content, assigns type/quality/topics/crime, writes to {source}_classified_content. Publish: Publisher filters classified content by route (quality, topics), publishes matching items to Redis channels (e.g. content:crime, content:news).`
