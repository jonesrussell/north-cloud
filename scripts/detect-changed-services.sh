...
GO_SERVICES=(auth classifier click-tracker crawler index-manager mcp-north-cloud pipeline publisher search source-manager nc-http-proxy)
FRONTEND_SERVICES=(dashboard search-frontend)
OTHER_SERVICES=(crime-ml mining-ml coforge-ml entertainment-ml anishinaabe-ml)
            ...
            for svc in $CHANGED_SERVICES_STR; do
                ...
                case "$svc" in
                    search)
                        ...
                        ;;
                    crime-ml|mining-ml|coforge-ml|entertainment-ml|anishinaabe-ml)
