...
rollback_services() {
  ...
  for svc in $services_to_restart; do
    case "$svc" in
      ...
      classifier)    check_health "classifier" "/health" "8070" 20 || rollback_healthy=false ;;
...
for svc in $SERVICES_TO_CHECK; do
  case "$svc" in
    ...
    source-manager)
      ...
      ;;
    classifier)
      check_health "classifier" "/health" "8070" 20 || { FAILED_CHECKS=$((FAILED_CHECKS + 1)); FAILED_SERVICES="$FAILED_SERVICES $svc"; }
      ;;
    publisher)
