...
owned_files:
- /home/jones/dev/indigenous-taxonomy/schema/treaties.yaml
- /home/jones/dev/indigenous-taxonomy/scripts/generate.py
- /home/jones/dev/indigenous-taxonomy/generated/go/taxonomy/treaties.go
- /home/jones/dev/indigenous-taxonomy/generated/go/taxonomy/treaties_test.go
priority: P1
tags: []
---
...
This is a cross-repo work package operating on `/home/jones/dev/indigenous-taxonomy/` (a sibling repo on disk). Other Phase A WPs build on this one via the same generator pipeline.
...
- The package's own conventions (additive=minor bump; rename/remove=major bump) are documented in `/home/jones/dev/indigenous-taxonomy/CLAUDE.md`.

## Branch Strategy

- Planning/base branch: `main`
- Merge target branch: `main` (within `/home/jones/dev/indigenous-taxonomy/`)
- Cross-repo: this WP operates outside the north-cloud monorepo. The agent must commit changes inside `/home/jones/dev/indigenous-taxonomy/` (separate git repo). A separate PR is opened against that repo's `main`. The lane worktree allocated by spec-kitty's `finalize-tasks` is informational; actual work happens in the sibling repo's checkout.
...
**Steps**:
1. Inspect existing schema files (`/home/jones/dev/indigenous-taxonomy/schema/regions.yaml`, `categories.yaml`, `dialects.yaml`) for conventions: top-level key, item shape, comment style.
2. Create `/home/jones/dev/indigenous-taxonomy/schema/treaties.yaml` with one entry per treaty 1 through 11:
   ```yaml
   treaties:
     - slug: "treaty:1"
...
**Files**:
- `/home/jones/dev/indigenous-taxonomy/schema/treaties.yaml` (new, ~80 lines).
...
**Steps**:
1. Read `/home/jones/dev/indigenous-taxonomy/scripts/generate.py` to understand the existing generator structure.
...
**Files**:
- `/home/jones/dev/indigenous-taxonomy/scripts/generate.py` (modified, +~50 lines).
...
2. Verify the output file at `/home/jones/dev/indigenous-taxonomy/generated/go/taxonomy/treaties.go`.
...
**Files**:
- `/home/jones/dev/indigenous-taxonomy/generated/go/taxonomy/treaties.go` (new, ~80 lines, generated).
...
**Steps**:
1. Create `/home/jones/dev/indigenous-taxonomy/generated/go/taxonomy/treaties_test.go`.
...
**Files**:
- `/home/jones/dev/indigenous-taxonomy/generated/go/taxonomy/treaties_test.go` (new, ~80 lines).
...
- All four subtasks complete and committed in `/home/jones/dev/indigenous-taxonomy/`.
...
- **Cross-repo coordination**: the agent must operate in `/home/jones/dev/indigenous-taxonomy/`. The lane worktree spec-kitty allocates is for the north-cloud repo; the agent should `cd /home/jones/dev/indigenous-taxonomy` and commit there.
...
This WP has no prerequisites. Begin immediately. The agent operates primarily in `/home/jones/dev/indigenous-taxonomy/` for code changes; the spec-kitty lane worktree is informational.
